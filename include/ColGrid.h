...
// Helper
typedef CollisionGrid ColGrid;
typedef CollisionGridIndex ColGridIndex;

/** Declarations **/
void cg_init_colgrid(ColGrid *cg);
void cg_free_colgrid(ColGrid *cg);
void cg_init_cell(CollisionGridCell *cell, int id, int init_inhabitants);
void cg_update_index(ColGrid *cg, CollisionGridIndex *cg_index, float pos_x, float pos_y, float width, float height);
CollisionGridCell cg_get_cell_id_at_pos(ColGrid *cg, float pos_x, float pos_y);
...
// TODO: void cg_remove_index(ColGrid *cg, CollisionGridIndex *cg_index) { }
...
void cg_get_inhabitants_in_cells(int *cell_ids, int num_cell_ids) {
	for (int i=0; i<num_cell_ids; i++) {
	}
}
